...
try {
    ...
    // Handle error appropriately, e.g., show notification or disable app features
}
...
async function fetchLicenses() {
    try {
        ...
        if (data && data.length > 0) {
            ...
            console.log('No se encontraron licencias en la base de datos');
            showNotification('No se encontraron licencias. Asegúrate de ejecutar el script SQL.', 'bg-yellow-500');
        }
    } catch (error) {
...
// Verifica si la tabla de licencias existe y tiene datos
async function checkLicensesTable() {
    try {
        console.log('Verificando tabla ms_licenses...');
        ...
        if (data && data.length > 0) {
            ...
            console.log('La tabla existe pero está vacía o no se encontró');
            return false;
        }
    } catch (error) {
        console.error('Error al verificar la tabla:', error);
...
function openAddModal() {
    ...
    // Populate license types dynamically
    const licenseTypeSelect = document.getElementById('licenseType');
    licenseTypeSelect.innerHTML = '';
    if (uiConfig && uiConfig.licenseTypes) {
        uiConfig.licenseTypes.forEach(type => {
    ...
    // Populate license statuses dynamically
    const licenseStatusSelect = document.getElementById('licenseStatus');
    licenseStatusSelect.innerHTML = '';
    if (uiConfig && uiConfig.licenseStatuses) {
        uiConfig.licenseStatuses.forEach(status => {
            const option = document.createElement('option');
            option.value = status;
...
// Helper function to check if the table exists on initialization
async function ensureSetup() {
    try {
        const tableExists = await checkLicensesTable();
        if (!tableExists) {
            console.log('Se recomienda ejecutar el script SQL en Supabase para crear la tabla');
            showNotification('Para usar Supabase, ejecuta el script SQL proporcionado', 'bg-yellow-500');
        }
    } catch (error) {
        console.error('Error durante la verificación inicial:', error);
    }
}
